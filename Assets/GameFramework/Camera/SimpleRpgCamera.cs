...
public class SimpleRpgCamera : MonoBehaviour
{
	...
	[HideInInspector]
	public bool m_bSmoothOffset = true;
	[HideInInspector]
	public float smoothOffsetSpeed = 5;
	[HideInInspector]
	public bool m_bRelativeOffset = true;								// Sets the offset relative to the target's forward rotation
	[HideInInspector]
	public bool m_bUseTargetAxis = false;								// When true, camera follows and rotates relative to target's rotation
	...
	[HideInInspector]
	public Vector2 originRotation = new Vector2();					// The initial rotation of the camera, rotation around y-axis/rotation around x-axis
	[HideInInspector]
	...
	private Vector2 _angle = new Vector2();// Rotation around y-axis/rotation around x-axis
    ...
    private List<Material> _fadedMats = new List<Material>();//已经透明了的材质，记录下来与_activeFadedMats，不在_activeFadedMats之中要复原
	private List<Material> _activeFadedMats = new List<Material>();//应透明的材质，在每帧的LateUpdate中更新，下一帧使用

	private bool _controllable = true;//总开关
	...
	void Update()
    {
        #region Calculate "_angle" for the rotation according to the input
        if (_controllable //总开关
            && allowRotation //可以旋转
            && ((mouseLook && (disableWhileUnlocked && !Cursor.visible || !disableWhileUnlocked)) //鼠标控制朝向
                  || (allowRotationLeft && Input.GetMouseButton(0)) //点鼠标左键
                  || (allowRotationMiddle && Input.GetMouseButton(2)) //点鼠标中键
                  || (allowRotationRight && Input.GetMouseButton(1)) //点鼠标右键
                  || allowTouch && Input.touchCount > 0) //有触摸
            )
		{
            GetInputFromTouchOrMouseAndJoystickForRotation();
            ...
            Cursor.visible = !(lockCursor && (mouseLook 
        ...
        else if (_controllable //总开关
                && allowRotationKeys  //可以旋转
                && (Input.GetKey(keyRotateUp) || Input.GetKey(keyRotateDown) || Input.GetKey(keyRotateLeft) || Input.GetKey(keyRotateRight))) //键盘有输入
		{
		...
		#region Calculate "distance" between target and camera consider zoom according to the input
		float scrollDirection = 0;
		if(_controllable)
		{
			if (allowZoom)
            {
                scrollDirection = GetInputFromMouseAndTouchForZoom();
			}

            if (allowZoomKeys) // Zoom key control
			{                
                scrollDirection = GetInputFromKeyboardForZoom();
			}
		}
        ...
        #region Calculate the position(distance and orientation) of camera and set it to "_focalPoint"
        if (_focalPoint)
		{
			if(_controllable && (allowEdgeMovement || allowEdgeKeys || allowMouseDrag))
			{
				...
				{
                    scrollVelocity = GetInputFromTouchForCarmeraPosition();
                }
				else
				{
                    scrollVelocity = GetInputFromMouseAndKeyboardForCarmeraPosition();
				}
        ...
        SetupFocalPointAndTarget();
		if(fadeObjects)
		{
            FadeOutObject();
		}
        FadeBackObjectInFrontCamera();
		#endregion
	}
	...
	void LateUpdate()
	{
		...
		{
            MoveCamera();
            FadeOutObjectInFrontCamera();
		}
	}

    //Rotation
    private void GetInputFromTouchOrMouseAndJoystickForRotation()
    {
        float inputX = 0;
        float inputY = 0;
    ...
    //Zoom
    private float GetInputFromMouseAndTouchForZoom()
    {
        float scrollDirection = 0;
        if (allowTouch && Application.isMobilePlatform)
        {
            if (Input.touchCount == 2)
            {
                ...
                if (t1.phase == TouchPhase.Began
                    ...
                    || (t1.phase == TouchPhase.Stationary && t2.phase == TouchPhase.Stationary))
                {
    ...
    private float GetInputFromKeyboardForZoom()
    {
        float scrollDirection = 0;
    ...
    //Camera position
    private Vector3 GetInputFromTouchForCarmeraPosition()
    {
        Vector3 scrollVelocity = new Vector3();
    ...
    private Vector3 GetInputFromMouseAndKeyboardForCarmeraPosition()
    {
        Vector3 scrollVelocity = new Vector3();
    ...
    private void UpdateFocalPointToTarget()
    {
        ...
        _currentOffset = m_bSmoothOffset ? Vector3.Lerp(_currentOffset, offset, smoothOffsetSpeed * Time.deltaTime) : offset;
    ...
    //Focal point and target setup
    private void SetupFocalPointAndTarget()
    {
        if (!_focalPoint)
        {
            CreateFocalPoint();
    ...
    private void FadeBackObjectInFrontCamera()
    {
    ...
    {
        #region Smoothly rotate the camera based on input angle
        Quaternion angleRotation = Quaternion.Euler(_angle.y, _angle.x, 0);
        Quaternion cameraRotation = (m_bUseTargetAxis ? target.rotation * angleRotation : angleRotation);
        ...
        #region Smoothly adjust the distance from the target
        ...
        #region Setup camera position and orientation
        ...
        _t.LookAt(_focalPoint.position, (m_bUseTargetAxis ? target.TransformDirection(Vector3.up) : Vector3.up));
        #endregion
    }
    private void FadeOutObjectInFrontCamera()
    {
        ...
        _activeFadedMats.Clear();//Reconstruct every frame
        ...
        foreach (RaycastHit alphaHit in hits)
        {
            ...
            if (hitRenderer)
            {
                ...
                foreach (Material mat in mats)
                {
                    ...
                    if (!_fadedMats.Contains(mat))//Whether it is new material to fade
                    {
                        _fadedMats.Add(mat);
                    }
                }
